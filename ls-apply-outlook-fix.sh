#!/usr/bin/env bash
# =============================================================================
# ls-apply-outlook-fix.sh — apply the Outlook/O365 scoping fix to the LIVE Little Snitch model
#
# Run:  sudo bash /Users/evw/dev/security/ls-apply-outlook-fix.sh [--dry-run]
#       --dry-run exports, runs ls-outlook-fix.py and shows the report, imports nothing
#
# Flow (mirrors ls-apply-tightening.sh / ls-apply-gmail-fix.sh):
#   1. export the LIVE model (root) — never edits a stale snapshot
#   2. ls-outlook-fix.py — delete Microsoft sentinel per-IP rules, add scoped
#      WebKit.Networking -> O365-domains tcp:443 allow
#   3. show the change report, require typing APPLY
#   4. restore-model; keep a backup of the pre-change model
#   5. re-export and verify: scoped O365 rule present, Microsoft sentinel rules gone
# =============================================================================
...
REPORT="$SCAN_DIR/ls-outlook-fix-report.txt"
BACKUP="$SCAN_DIR/ls-model-pre-outlook-fix-$(date +%s).json"
...
mkdir -p "$SCAN_DIR"
WORK=$(mktemp -d /var/tmp/ls-outlook-fix.XXXXXX)
...
echo "[2/5] Running outlook fix..."
guard_run "ls-outlook-fix" python3 "$SEC_DIR/ls-outlook-fix.py" "$WORK/model.json" "$WORK/fixed.json" --report "$REPORT" || true
...
def has_o365(proc):
    return any(r.get('action')=='allow' and r.get('process')==proc
               and 'outlook.com' in (r.get('remote-domains') or []) for r in rules)
webkit = has_o365('identifier.APPLE/com.apple.WebKit.Networking')
safari = has_o365('identifier.APPLE/com.apple.Safari')
stale = sum(1 for r in rules if str(r.get('notes','')).startswith('[AUTO-EVW-LS] sentinel-deny')
            and '(Microsoft Corporation' in str(r.get('notes','')))
print('webkit-rule=%s safari-rule=%s stale-microsoft=%d' % (webkit, safari, stale))
sys.exit(0 if webkit and safari and stale == 0 else 1)"; then
    echo "[✓] Outlook fix applied and verified. Report: $REPORT"
else
