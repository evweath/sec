...
add "$S" "ls-scope-key-resources.py"                           "[MOD]"      "Scope LS model JSON: signed-process+domain :443 rules for key resources, strip any-process IP allows"
add "$S" "ls-apply-scope-key-resources.sh"                     "[MOD,SUDO]" "Export live model, scope key resources to signed processes, import after typing APPLY (--dry-run/--yes)"
add "$S" "ls-gmail-fix.py"                                     "[MOD]"      "Scope LS model JSON: WebKit+google-mail domains :443, delete 1e100 sentinel per-IP rules"
add "$S" "ls-apply-gmail-fix.sh"                               "[MOD,SUDO]" "Export live model, apply gmail scoping fix, import after typing APPLY (--dry-run for report only)"
add "$S" "ls-outlook-fix.py"                                   "[MOD]"      "Scope LS model JSON: WebKit+O365 domains :443, delete Microsoft sentinel per-IP rules"
add "$S" "ls-apply-outlook-fix.sh"                             "[MOD,SUDO]" "Export live model, apply Outlook/O365 scoping fix, import after typing APPLY (--dry-run for report only)"
...
add "$S" "evw-security-audit-setup.sh"                 "[SVC,SUDO]" "Install the audit to run at every boot (root, full) + login (user); --uninstall removes"
...
add "$S" "scripts/evw-auto-ls-sync.sh"                 "[MOD,SUDO]"  "[AUTO-EVW] Merge queued auto-block deny rules into Little Snitch (backup first)"
add "$S" "scripts/ls-deny-remote.py"                   "[MOD,SUDO]"  "[AUTO-EVW] Plant blanket LS denies: 16 macOS remote-access daemons (verified IDs)"
add "$S" "scripts/ls-deny-screenshare.py"              "[MOD,SUDO]"  "[AUTO-EVW] Plant blanket LS denies: 24 screen-sharing/remote-desktop programs"
