...
#    ./run_migration.sh --gui              # launch the Tkinter GUI
# =============================================================================
...
install_deps() {
    ...
    info "Installing dependencies (requests, certifi, python-dotenv) ..."
    "$pip" install --quiet --upgrade pip
    "$pip" install --quiet requests certifi python-dotenv
    local ver
...
if [[ ! -d "$VENV_DIR" ]]; then
    ...
    # Venv exists — just check requests + dotenv are installed
    if ! "$VENV_DIR/bin/python" -c "import requests, dotenv" &>/dev/null; then
...
# ── Check for --gui flag ──────────────────────────────────────────────────────
if [[ "${1:-}" == "--gui" ]]; then
    GUI_SCRIPT="$SCRIPT_DIR/shopify_gui.py"
    if [[ ! -f "$GUI_SCRIPT" ]]; then
        error "shopify_gui.py not found at: $GUI_SCRIPT"
        exit 1
    fi
    info "Launching GUI ..."
    # Ensure tkinter is available
    if ! "$VENV_DIR/bin/python" -c "import tkinter" 2>/dev/null; then
        warn "tkinter not found in venv — trying system python ..."
        if command -v python3 &>/dev/null && python3 -c "import tkinter" 2>/dev/null; then
            exec python3 "$GUI_SCRIPT"
        else
            error "tkinter is not available. On Linux: sudo apt install python3-tk"
            exit 1
        fi
    fi
