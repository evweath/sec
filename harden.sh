...
#   • Pins DNS to 1.1.1.1 / 1.0.0.1 / 8.8.8.8 / 9.9.9.9 on every active service
#   • Installs: lynis, nmap, gpg, ykman, clamav, tor, dnscrypt-proxy
#   • Installs (cask): tor-browser, signal, little-snitch
...
# ────────────────────────────────────────────────────────────────────────────
# 8. DNS — pinned resolvers (Cloudflare, Google, Quad9)
# ────────────────────────────────────────────────────────────────────────────
section "8. DNS → 1.1.1.1 / 1.0.0.1 / 8.8.8.8 / 9.9.9.9"
while IFS= read -r svc; do
    [[ -n "$svc" && "$svc" != \** ]] || continue
    log "  $svc"
    try sudo networksetup -setdnsservers "$svc" 1.1.1.1 1.0.0.1 8.8.8.8 9.9.9.9
...
try brew install lynis nmap gpg pinentry-mac ykman clamav tor dnscrypt-proxy pipx
# stronghold left homebrew-core — upstream publishes it via PyPI instead
try pipx install stronghold
try brew install --cask tor-browser signal little-snitch
...
if [[ -n "$WAZ_ARCH" ]]; then
    ...
    # The packages.wazuh.com directory index returns 403, so resolve the latest
    ...
    if [[ -n "$WAZ_VER" ]]; then
        ...
        warn "Could not determine latest Wazuh version — install manually from packages.wazuh.com"
    fi
else
...
warn "  3. Enroll YubiKey on every account that matters (Apple ID, email, GitHub, banks)"
warn "  4. Open Tor Browser at least once to seed bridges if needed"
warn "  5. Finish Wazuh setup: sudo bash ~/dev/security/evw-wazuh-setup.sh [MANAGER_IP] (or remove if unused)"
warn "  6. Configure Little Snitch rules on first launch (default-deny outbound)"
warn ""
