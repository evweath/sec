...
REAL_USER="${SUDO_USER:-$(logname 2>/dev/null || echo "ew")}"
...
PLIST_PATH="/Library/LaunchDaemons/com.apple.thermald.plist"
...
PLIST_EOF
    ...
    if launchctl load -w "$PLIST_PATH" 2>/dev/null; then
        sleep 2
        if launchctl list com.apple.thermald &>/dev/null; then
...
echo "    2. Check daemon:  sudo launchctl list com.apple.thermald"
